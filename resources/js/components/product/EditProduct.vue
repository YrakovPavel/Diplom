                    ...
                    <div class="col-xl-6 col-lg-8 col-md-9 wow fadeInUp animated">
                        <div class="login-register-form">
                            <div class="divider d-flex align-items-center my-2">
                                <h3 class="text-center fw-bold mb-0">Редактировать товар</h3>
                            </div>
                            <form class="common-form">
                                ...
                                <p></p>
                                <input @click.prevent="addProduct" type = "submit" value = "Редактировать" class = "btn btn-primary">
                            </form>
                        </div>
                    </div>
                </div>
...
export default{
    ...
    created(){
        ...
        axios.post('/api/getProduct', {id: this.id}).then(response =>{
            ...
            this.categories = [],
            axios.post('/api/getCharacteristics', {id: this.id}).then(response=>{
                let chars = response.data;
                if (chars){
                    this.charFields = [];
                    chars.forEach(element => {
                        this.charFields.push({char: element.name, value: element.value});
                    });
                }
            })
        })
    ...
    methods:{
        addProduct(){
            ...
            axios.post('/api/editProduct', formProduct)
                .then(response =>{
                    ...
                    formChar.append('product_id', this.id);

                    axios.post('/api/deleteCharacteristics', {product_id: this.id});

                    axios.post('/api/addCharacteristics', formChar).then(response =>{
        ...
        onCategoryChange(e){
            this.category_id = parseInt(e.currentTarget.id);
        },
